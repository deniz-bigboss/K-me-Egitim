...
When driving the site with Playwright in a sandbox, these cause false failures:
...
Outbound HTTPS to arbitrary sites is proxied and usually 403s, but **raw sockets to
kumeegitim.com on ports 80 and 443 work**, so the live site can be verified directly with
`socket` + `ssl`. TLS interception makes certificate checks unreliable (issuer shows
"Anthropic") — don't draw conclusions about the real cert from inside the sandbox.

## Gotchas

- After a container restart the working copy may reappear on the stale branch
